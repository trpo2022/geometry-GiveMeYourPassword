...
string toCorrect(string str) {
  ...
  while (str.find("  ", 0) != -1) str.replace(str.find("  ", 0), 2, " ");
...
int checkErrors(string str) {
  ...
  if (str.find(")", 0) == -1) {
    cout << str << endl;
    for (int i = 0; i < str.size() - 1; i++) cout << " ";
    cout << "^" << endl
         << "Error at column " << str.size() - 1 << ": expected ')'" << endl
         << endl;
    return 0;
  }
  ...
  if (str[str.size() - 1] != ')') {
    cout << str << endl;
    for (int i = 0; i <= str.find(")") + 1; i++) cout << " ";
    cout << "^" << endl
         << "Error at column " << str.find(")") + 2 << ": unexpected token"
         << endl
         << endl;
    return 0;
  }
  ...
  for (int i = str.find("(") + 1; i < str.find(")"); i++)
    if (!isdigit(str[i]) && str[i] != '.' && str[i] != ',' && str[i] != ' ' &&
      ...
      cout << "^" << endl
           << "Error at column " << i << ": expected '<double>'" << endl
           << endl;
      return 0;
    }
...
void getData(string str, Circle* data, int* size) {
  ...
  int k = 0;
  for (int i = str.find("(") + 1; i <= str.find(")"); i++) {
    if (isdigit(str[i]) || str[i] == '.' || str[i] == '-') digit += str[i];
    if (isspace(str[i]) || str[i] == ')') {
      double convertDigit = atof(digit.c_str());
      switch (k) {
        case 0:
          data[*size].x = convertDigit;
          break;
        case 1:
          data[*size].y = convertDigit;
          break;
        case 2:
          data[*size].R = convertDigit;
          break;
      }
      k++;
      digit.clear();
